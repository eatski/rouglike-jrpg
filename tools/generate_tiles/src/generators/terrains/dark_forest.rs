...
pub fn generate_dark_forest(output_dir: &Path) {
    let mut img = new_image();

    let base = Rgba([50, 30, 60, 255]);
    let dark = Rgba([30, 15, 45, 255]);
    let light = Rgba([70, 40, 80, 255]);
    let trunk = Rgba([50, 25, 30, 255]);
    ...
    for (i, (tx, ty)) in tree_positions.iter().enumerate() {
        ...
        for dy in 0..4u32 {
            ...
            for dx in 0..=width {
                ...
                if leaf_y < TILE_SIZE {
                    if lx1 < TILE_SIZE {
                        let c = if pixel_noise(lx1, leaf_y, 210 + i as u32) < 0.3 {
                            light
                        } else {
                            dark
                        };
                    ...
                    if lx2 < TILE_SIZE && lx2 != lx1 {
                        let c = if pixel_noise(lx2, leaf_y, 220 + i as u32) < 0.3 {
                            light
                        } else {
                            dark
                        };
    ...
    for y in 0..TILE_SIZE {
        for x in 0..TILE_SIZE {
            if img.get_pixel(x, y) == &base && pixel_noise(x, y, 230) < 0.2 {
                let c = if pixel_noise(x, y, 231) < 0.5 { dark } else { light };
                img.put_pixel(x, y, c);
            }
        }
    }

    save_image(&img, output_dir, "dark_forest.png");
}
