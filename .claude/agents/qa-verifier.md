...
## screenshot-verifier との連携

UI関連の変更（`*-ui/` crate、レイアウト、色、テキスト表示など）を検出した場合、手動確認の促しに加えて **screenshot-verifier エージェントの利用を推奨** する。スクリーンショットによる視覚的な検証で、UIリグレッションを早期に発見できる。
