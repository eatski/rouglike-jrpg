...
**UI機能層（Bevy依存）**:
- **world-ui**: ワールドマップシーン・入力・描画・エンカウントシステム・仲間候補マーカー表示・フィールドHUDにレベル表示（HudNameTextコンポーネント）
...
- **battle-ui**: 戦闘シーン・入力（クラス別呪文選択制限、個人インベントリ使用）・表示（呪文リスト、無効コマンド灰色表示、アイテムなし時「どうぐ」灰色）・フィールドメニュー（確認キー→トップメニュー（じゅもん/どうぐ）→キャスター/メンバー選択→呪文/アイテム選択→ターゲット選択→結果表示）・勝利時経験値獲得メッセージ・レベルアップメッセージ表示・HUDにレベル表示
