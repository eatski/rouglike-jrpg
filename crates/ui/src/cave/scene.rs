...
use game::cave::{generate_cave_map, CaveMapData, CaveTerrain, CAVE_HEIGHT, CAVE_WIDTH};

use crate::components::{Boat, MapTile, MovementLocked, PendingMove, Player, TilePosition};
use crate::constants::TILE_SIZE;
use crate::rendering::{spawn_boat_entities, BoatSpawnsResource, TileTextures};
use crate::resources::MovementState;
use crate::smooth_move::SmoothMove;
use crate::bounce::Bounce;
use crate::tile_pool::{create_tile_pool, PooledTile};
...
pub fn setup_cave_scene(
    ...
    commands.remove_resource::<crate::tile_pool::TilePool>();
...
pub fn cleanup_cave_scene(
    ...
    if let Ok((entity, mut tile_pos, mut transform)) = player_query.single_mut() {
        ...
        // フィールドのワールド座標に戻す
        let origin_x = -(game::map::MAP_WIDTH as f32 * TILE_SIZE) / 2.0 + TILE_SIZE / 2.0;
        let origin_y = -(game::map::MAP_HEIGHT as f32 * TILE_SIZE) / 2.0 + TILE_SIZE / 2.0;
